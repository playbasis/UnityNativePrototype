...
It demonstrates how to integrate Unity with UIKit and the native Android UI.

## Concept

Developers create a static UI library (NativeProjectLib) that contains application's native UI and logic that will be used in native application, then (optional) create a bootstrap native application (NativeProjectBootstrap) to test against the static UI library. Finally developers copy result static UI library that has been tested to Assets/Plugin/iOS or Assets/Plugin/Android folder in Unity. When build to native project from Unity, it will automatically copies those files and be able to run on simulator or device.
